...
#define DS18B20_TIMER_TST 0
#define DS18B20_TIMER_READ 1
#define DS18B20_TIMER_UPLOAD 2
#define DS18B20_TIMER_REQUEST 3
...
public:
	...
	DS18B20(int pin, const byte addres[8])      // a 4.7K resistor is necessary for 2 wire power steal
	{
		_pin = pin;
		setupPin();
		setup( );
	...
	DS18B20(int pin, const byte addres[8], int ID)     // a 4.7K resistor is necessary for 2 wire power steal
	{
		_pin = pin;
		setupPin();
		id = ID;
	...
	~DS18B20()
	{
 	...
 	volatile unsigned long 	timers2[DS18B20_TIMER_COUNT];
	...
	void nextTimer2( int id){ nextTimerMillis2(id, sensorFrequency_s * 1000L );}	
	void nextTimer2( int id, int periode){ nextTimerMillis2(id, periode * 1000L );}	
	void nextTimerMillis2( int id, unsigned long periode);
